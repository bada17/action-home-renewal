...
/* ───── 히어로 ─────
   활동·이슈와 같은 짙은 남색 문입니다. 화면을 오갈 때 같은 문이 반복돼야
   한 사이트로 읽힙니다. */
#lib .hero{{background:var(--navy); color:#fff; padding:clamp(40px,5vw,72px) 0}}
#lib .crumb{{font-size:12.5px; font-weight:600; color:#7fb7ce; margin-bottom:clamp(22px,2.6vw,34px)}}
#lib .crumb a{{color:#7fb7ce}}
#lib .crumb a:hover{{text-decoration:underline; text-underline-offset:3px}}
#lib .crumb span{{opacity:.45; margin:0 6px}}
#lib .hero h1{{font-size:clamp(26px,4vw,42px); font-weight:700; line-height:1.3; text-wrap:balance}}
#lib .hero h1 em{{display:block; font-style:normal; color:#5fcdf5}}
#lib .hero .by{{margin-top:15px; font-size:clamp(14.5px,1.2vw,16.5px); color:#a8cddc; max-width:56ch; line-height:1.7}}

/* 히어로 밑 숫자 셋 — 이 자료실이 무엇을 얼마나 담고 있는지 한 줄로. */
#lib .facts{{display:grid; grid-template-columns:repeat(3,1fr); margin-top:clamp(26px,3vw,38px);
  border:1px solid rgba(255,255,255,.18)}}
#lib .fact{{padding:16px 18px; border-left:1px solid rgba(255,255,255,.18)}}
#lib .fact:first-child{{border-left:none}}
#lib .fact b{{display:block; font-family:GmarketSans,sans-serif; font-size:clamp(21px,2.4vw,30px);
  font-weight:700; line-height:1.15; color:#fff; letter-spacing:-.03em}}
#lib .fact span{{display:block; margin-top:5px; font-size:12.5px; font-weight:600; color:#a8cddc}}
...
@media(max-width:560px){{
  ...
  #lib .hero{{padding:30px 0}}
  #lib .facts{{grid-template-columns:1fr}}
  #lib .fact{{border-left:none; border-top:1px solid rgba(255,255,255,.18)}}
