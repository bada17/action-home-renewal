...
(function () {
  ...
  /* 날짜가 늦은 것부터. 날짜가 같거나 없으면 글 번호가 큰 것이 새 글입니다.
     ★ 게시판에 그려진 차례를 그대로 믿으면 안 됩니다 — 맨 위에 고정된 공지가
       있으면 그 둘이 올라옵니다. 공지는 대개 날짜가 오래돼 이 줄세우기에서 내려갑니다.
       (2026-09-10. 그 전에는 앞에서부터 둘을 그냥 잘라 썼습니다.) */
  function newestFirst(rows) {
    return rows
      ...
      .sort(function (a, b) {
        ...
        if (da !== db) {
          if (!da) return 1;               // 날짜 없는 것은 뒤로
          if (!db) return -1;
          return da < db ? 1 : -1;         // 2026.09.10 꼴이라 글자 비교로 충분합니다
        }
        var ia = +a.r.idx || 0, ib = +b.r.idx || 0;
        if (ia !== ib) return ib - ia;
        return a.i - b.i;                  // 그래도 같으면 그려진 차례대로
      })
  ...
  function pickFromBoard() {
    ...
    return newestFirst(mine.length ? mine : all).slice(0, 2);
  }
  ...
  function fromRss() {
    ...
    fetch('/rss', { credentials: 'same-origin' })
      ...
      .then(function (xmlText) {
        ...
        draw(newestFirst(rows).slice(0, 2));
      })
